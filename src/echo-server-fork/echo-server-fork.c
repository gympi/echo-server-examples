...
int main(int argc, char *argv[]) {
	...
	while (true) {
        ...
        pid_t pid = fork();
        if (pid == 0) { // handle client connection in a separate process
            close(server_fd);
